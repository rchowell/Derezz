...
def index(catalog: Catalog, filepath: str) -> None:
    """Indexes the video at the given filepath (or url) in the provided Catalog."""
    # Video metadata
    v_uuid = uuid.uuid4().bytes
    v_name = filepath # basename
    v_location = filepath
    ...
    df = df.select(*items({
        ...
        "f_number": df["frame_number"],
        "ft_label": df["label"],
        "ft_image": to_jpg(df["data"]), # write amplification
